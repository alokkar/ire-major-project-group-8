...
def main():
	...
	# print(data)
	# data=np.append(data,tweet_lengths,axis=1)
	# data=data+tweet_lengths
	# print(data)
	# return
	...
	nb_clf_ag_wl=nb_classifier_withLen(data,labels_AG)
	nb_clf_tr_wl=nb_classifier_withLen(data,labels_TR)
	svm_clf_ag_wl= svm_classifier_withLen(data,labels_AG)
	svm_clf_tr_wl= svm_classifier_withLen(data,labels_TR)
	lr_clf_ag_wl=logreg_classifier_withLen(data,labels_AG)
	lr_clf_tr_wl=logreg_classifier_withLen(data,labels_TR)
	


	file = 'dev_en.tsv'
	test_data,labels_HS_test,labels_TR_test,labels_AG_test = load_data(file)
	print("NB without length")
	testAG(file,nb_clf_ag)
	testTR(file,nb_clf_tr)
	print("SVM without length")
	testAG(file,svm_clf_ag)
	testTR(file,svm_clf_tr)
	print("LogReg without length")
	testAG(file,lr_clf_ag)
	testTR(file,lr_clf_tr)
	print("NB with length")
	testAG(file,nb_clf_ag_wl)
	testTR(file,nb_clf_tr_wl)
	print("SVM with length")
	testAG(file,svm_clf_ag_wl)
	testTR(file,svm_clf_tr_wl)
	print("LogReg with length")
	testAG(file,lr_clf_ag_wl)
	testTR(file,lr_clf_tr_wl)
